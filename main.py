...
async def fetch_image_fast(bot, file_id, idx):
    ...
    try:
        ...
        # Keep original resolution for super HD
...
def generate_smart_pdf(records, downloaded_images, title, summary_text, record_ids):
    """Generate PDF with native high-res images and minimal margins"""
    pdf_buffer = BytesIO()
    ...
    for idx, rec in enumerate(records):
        ...
        if rec['type'] == 'photo' and idx in downloaded_images:
            ...
            if img_path and os.path.exists(img_path):
                try:
                    # Use ReportLab's ImageReader for native High-Res (no PIL re-scaling)
                    ...
                    # Draw High-Res Image
                    c.drawImage(img_reader, x_pos, y_position - final_h, 
                               width=final_w, height=final_h, mask='auto')
...
# ==================== PDF GENERATION WITH CRASH FIXES ====================
async def generate_and_send_pdf_with_summary(update: Update, context: ContextTypes.DEFAULT_TYPE, 
    ...
    try:
        ...
        try:
            ...
            if status_msg:
                await status_msg.edit_text(f"❌ મોકલવામાં ભૂલ: {str(e)[:100]}")
            return
...
async def date_pdf_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        ...
        if dates:
            ...
            await update.message.reply_text("📅 *Select a date:*\n\nOr use `/date_pdf YYYY-MM-DD`", parse_mode="Markdown", reply_markup=reply_markup)
        else:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_daily": await daily_pdf(update, context)
        ...
        await query.message.edit_text("📅 *Date Range PDF*\n\nUsage: `/daterange YYYY-MM-DD YYYY-MM-DD`\nExample: `/daterange 2026-07-01 2026-07-29`")
...
async def main():
    ...
    application.add_error_handler(error_handler) # <-- NEW SAFETY LAYER
